...
from pysdmx.model import Agency, Annotation, Concept, ConceptScheme


@pytest.fixture
def cs():
    c = Concept("A", name="Annual")
    return ConceptScheme(
        "SDMX_CS",
        name="SDMX Concepts",
        agency="SDMX",
        description="Generic concepts",
        version="42.0",
        items=[c],
...
@pytest.fixture
def cs_no_name():
    return ConceptScheme("CL_FREQ", agency="BIS")


def test_cs(cs: ConceptScheme):
    sjson = JsonConceptScheme.from_model(cs)
...
def test_cs_org(cs_org: ConceptScheme):
    sjson = JsonConceptScheme.from_model(cs_org)
...
def test_cs_no_name(cs_no_name):
    with pytest.raises(errors.Invalid, match="must have a name"):
        JsonConceptScheme.from_model(cs_no_name)
