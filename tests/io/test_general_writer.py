...
from pysdmx.model import Component, Role, Concept, Components, Schema
...
GEN_STRUCTURE = Schema(
            ...
            components=Components(
                [
                    Component(
                        id=id_,
                        role=Role.DIMENSION if id_ in DIMENSIONS else
                        Role.ATTRIBUTE if id_ in ATTRIBUTES else
                        Role.MEASURE,
...
def extension(format_):
    return "csv" if format_ in [Format.DATA_SDMX_CSV_1_0_0, Format.DATA_SDMX_CSV_2_0_0] else "xml"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Format.DATA_SDMX_ML_2_1_GEN, XML_2_1_PATH, "gen_all.xml", {"dimension_at_observation": {}}),
        (Format.DATA_SDMX_ML_2_1_STR, XML_2_1_PATH, "str_all.xml", {}),
    ],
)
def test_write(format_, test_path, reference_file, params, reference, output_path):
