...
def bis_metadata_sample():
    url = 'https://stats.bis.org/api/v1/dataflow/BIS/WS_CBS_PUB/1.0?references=descendants&detail=full'
    return read_sdmx_from_url(url)


@pytest.fixture
def bis_data_sample():
    url = 'https://stats.bis.org/api/v1/data/BIS%2CWS_CBS_PUB%2C1.0/all/all?startPeriod=2024-Q1&detail=full'
    return read_sdmx_from_url(url)
...
def test_sdmx_to_vtl(bis_metadata_sample, bis_data_sample, bis_script_sample):
    ...
    data_structure = metadata_result['DataStructures']['DataStructure=BIS:BIS_CBS(1.0)']
    ...
    vtl_data_structure = change_component_type(vtl_data_structure, 'OBS_VALUE', 'Number')
    datapoints = {"BIS_CBS": data_result['DataFlow=BIS:WS_CBS_PUB(1.0)'].data}

    run_result = run(script=bis_script_sample, data_structures=vtl_data_structure,
                     datapoints=datapoints, return_only_persistent=True)
