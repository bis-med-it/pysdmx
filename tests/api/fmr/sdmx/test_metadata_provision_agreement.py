...
@pytest.fixture
def fmr():
    return RegistryClient("https://registry.sdmx.org/sdmx/v2/")


@pytest.fixture
def async_fmr() -> AsyncRegistryClient:
    return AsyncRegistryClient("https://registry.sdmx.org/sdmx/v2/")
...
@pytest.fixture
def body():
    with open("tests/api/fmr/samples/pa/mpa.json", "rb") as f:
        return f.read()


def test_returns_provision_agreement(respx_mock, fmr, query, body):
    """get_metadata_provision_agreement() should return an mpa."""
    checks.check_metadata_provision_agreements(respx_mock, fmr, query, body)
