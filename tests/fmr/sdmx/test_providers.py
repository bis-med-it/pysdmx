...
def query(fmr: RegistryClient):
    ...
    agency = "BIS"
    return f"{fmr.api_endpoint}{res}{agency}?references=none"
...
@pytest.fixture()
def body():
    with open("tests/fmr/samples/orgs/providers.json", "rb") as f:
        return f.read()


@pytest.fixture()
def flowbody():
    with open("tests/fmr/samples/orgs/providersflows.json", "rb") as f:
        return f.read()
